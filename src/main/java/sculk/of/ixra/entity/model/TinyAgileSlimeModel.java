package sculk.of.ixra.entity.model;

import software.bernie.geckolib.model.GeoModel;

import sculk.of.ixra.entity.TinyAgileSlimeEntity;

import net.minecraft.resources.ResourceLocation;

public class TinyAgileSlimeModel extends GeoModel<TinyAgileSlimeEntity> {
	@Override
	public ResourceLocation getAnimationResource(TinyAgileSlimeEntity entity) {
		return ResourceLocation.parse("sculks_of_arda:animations/sculkslime.animation.json");
	}

	@Override
	public ResourceLocation getModelResource(TinyAgileSlimeEntity entity) {
		return ResourceLocation.parse("sculks_of_arda:geo/sculkslime.geo.json");
	}

	@Override
	public ResourceLocation getTextureResource(TinyAgileSlimeEntity entity) {
		return ResourceLocation.parse("sculks_of_arda:textures/entities/" + entity.getTexture() + ".png");
	}

}
